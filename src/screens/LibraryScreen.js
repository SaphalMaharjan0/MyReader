...
export default function LibraryScreen({ navigation }) {
   ...
   const handleToggleSetting = async (key, value) => {
      ...
      if (key === 'darkMode') setIsDarkMode(value);
   ...
   const handleClearLibrary = async () => {
      const { clearBooks } = require('../utils/storage');
      Alert.alert(
         'Clear Library',
   ...
   const handleImportBook = async () => {
      ...
      try {
         ...
         if (!result.canceled && result.assets && result.assets.length > 0) {
            ...
            // Ensure it's an epub or pdf
            if (!file.name.toLowerCase().endsWith('.epub') && !file.name.toLowerCase().endsWith('.pdf')) {
               Alert.alert('Invalid File', 'Please select a valid EPUB or PDF file.');
               setIsPicking(false);
               return;
            }
            ...
            const newBook = {
               ...
               uri: newUri,
               type: file.mimeType || (file.name.endsWith('.epub') ? 'application/epub+zip' : 'application/pdf'),
               dateAdded: new Date().toISOString(),
               coverUri: null
            };

            let importedBook = { ...newBook };
            if (newBook.type.includes('epub') || newBook.name.toLowerCase().endsWith('.epub')) {
               try {
            ...
            const displayName = importedBook.name.replace(/\.epub$/i, '').replace(/\.pdf$/i, '');
   ...
   const renderBookItem = ({ item }) => {
      ...
      const displayName = item.customTitle || item.name.replace(/\.epub$/i, '').replace(/\.pdf$/i, '');
            ...
            <View style={StyleSheet.absoluteFill} zIndex={1000}>
               ...
               <Animated.View style={[
                  ...
                  <ScrollView style={styles.drawerScroll}>
                     ...
                     <TouchableOpacity style={styles.drawerSettingsItem} onPress={() => { closeDrawer(); navigation.navigate('CustomizeToolbar'); }}>
                     ...
                     <TouchableOpacity style={styles.drawerSettingsItem} onPress={() => { closeDrawer(); handleImportBook(); }}>
                        <Ionicons name="add-circle-outline" size={20} color={isDarkMode ? '#AAA' : '#555'} style={styles.drawerItemIcon} />
                        <View style={styles.drawerItemTextContainer}>
                           <Text style={[styles.drawerItemText, { color: isDarkMode ? '#FFF' : '#333' }]}>Import Book</Text>
                           <Text style={styles.drawerItemSubtext}>EPUB or PDF</Text>
                        </View>
                     </TouchableOpacity>

                     <TouchableOpacity style={styles.drawerSettingsItem} onPress={async () => {
                        closeDrawer();
                        try {
                     ...
                     <TouchableOpacity style={styles.drawerSettingsItem} onPress={() => { closeDrawer(); handleClearLibrary(); }}>
                     ...
                     <TouchableOpacity style={styles.drawerSettingsItem} onPress={() => { closeDrawer(); handleClearCache(); }}>
                  ...
                  <View style={[styles.drawerFooter, { borderTopColor: isDarkMode ? '#333' : '#E0E0E0' }]}>
                     ...
                     <View style={styles.drawerStatsRow}>
                        ...
                        <Text style={[styles.drawerStatsValue, { color: isDarkMode ? '#FFF' : '#333' }]}>{books.length}</Text>
                     </View>
                     <View style={styles.drawerStatsRow}>
                        <Text style={styles.drawerStatsLabel}>Finished:</Text>
                        <Text style={[styles.drawerStatsValue, { color: isDarkMode ? '#FFF' : '#333' }]}>
                           {books.filter(b => (b.progress || 0) >= 1.0).length}
                        </Text>
                     </View>
                     <View style={styles.drawerStatsRow}>
                        <Text style={styles.drawerStatsLabel}>Reading:</Text>
                        <Text style={[styles.drawerStatsValue, { color: isDarkMode ? '#FFF' : '#333' }]}>
                           {books.filter(b => (b.progress || 0) > 0 && (b.progress || 0) < 1.0).length}
                        </Text>
                     </View>
                  </View>
               </Animated.View>
